...
public class Persona extends Thread {
    ...
    public void run() {
        if (parqueX.parqueEstaAbierto()) {
            ...
            if (parqueX.personaIntentarEntrarParque()) {
                ...
                parqueX.salirDeParque(); // LA SALIDA ES POR EL MISMO LUGAR
            } else {
                System.out.println("EL PARQUE ESTABA LLENO, LA PERSONA SE FUE....");
            }
        } else {
    ...
    // METODOS DE ATRACCIONES DEL PARQUE
            ...
            if (parqueX.montaniaRusaAbierto()) {
                if (parqueX.ingresarAMontania(this)) {
                    ...
                    if (parqueX.esperaArranque(this)) {
                        ...
                        System.out
                                .println("Una persona espero mucho tiempo y se cansó, se ha bajado de la montaña rusa");
                    }
                } else {
        ...
        if (parqueX.teatroAbierto()) {
            try {
                if (parqueX.ingresarATeatro()) {
                    ...
                    fueRealizada = true;
                    if (parqueX.salirTeatro())
                        System.out.println("Una persona salio del teatro");
                    else
                        System.out.println("Una persona se canso de esperar dentro del teatro y se fue...");
                } else {
                    System.out.println("Una persona se canso de esperar afuera del teatro y se fue...");
                }
            } catch (Exception e) {
            }
        }
    ...
    // METODOS DE LUGARES DE SHOPPING DEL PARQUE
    public void canjearPremio() {
        if (cantTickets != 0) {
            parqueX.personaIngresaTienda();
            this.cantTickets = parqueX.canjearPremios(cantTickets);
            System.out.println("Persona " + nombre + " canjeo sus puntos ");
        }
        try {
            Thread.sleep(1000);
        } catch (Exception e) {
